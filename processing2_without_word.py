...
def build_vec_mod():
    ...
    for x in range(1,400):
        ...
        diction2.update({x: temp_dic})

        termsInAfile.clear()
    for a in range(1,400):
        temp_dic2 = diction2.get(a).copy()
        diction2.get(a).clear()

        for y in tokines:
            if y not in temp_dic2.keys():
                diction2.get(a).update({y: 0.0})
            else:
                diction2.get(a).update({y: temp_dic2[y]})

    f1 = open("vector model2.txt", "w")
    f1.write(str(diction2))
    f1.close()
